...
/**
 * Generate a unique document ID with the format doc-{timestamp}-{random}
 *
 * This format ensures document IDs are:
 * - Unique and sortable by creation time
 * - Follow the pattern /^doc-\d+-[a-z0-9]+$/
 * - Compatible with existing test expectations
 *
 * @returns A unique document ID string
 *
 * @example
 * const docId = generateDocumentId() // => "doc-1735382445123-8c08u65"
 */
export function generateDocumentId(): string {
  const timestamp = Date.now()
  const randomSuffix = nanoid(6).toLowerCase() // 6 characters for uniqueness, lowercase only
