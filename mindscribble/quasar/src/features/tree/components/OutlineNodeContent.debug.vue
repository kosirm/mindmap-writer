...
import { createKeyboardHandler, createNavigationHandler } from '../composables/useOutlineKeyboardHandlers.debug' // Use debug version
  ...
  const navigationHandler = createNavigationHandler({
    onUpArrow: () => {
      if (navigation) {
        ...
        console.log('🟡 [DEBUG NAVIGATION MODE] Previous node:', prevNode)
        if (prevNode) {
    ...
    onDownArrow: () => {
      if (navigation) {
        ...
        console.log('🟠 [DEBUG NAVIGATION MODE] Next node:', nextNode)
        if (nextNode) {
...
function navigateToNode(nodeId: string, cursorPosition: 'start' | 'end') {
  console.log(`🚀 [DEBUG NAVIGATE TO NODE] ${props.node.id} -> ${nodeId} (${cursorPosition})`)
    ...
    editorProps: {
      handleKeyDown: createKeyboardHandler({
        ...
        onUpArrowAtFirstLine: () => {
          console.log('🔵 [DEBUG EDIT MODE] UP ARROW at first line triggered for node:', props.node.id)
          if (navigation) {
            const prevNode = navigation.getPreviousNode(props.node.id)
            console.log('🔵 [DEBUG EDIT MODE] Previous node:', prevNode)
            if (prevNode) {
              console.log('🔵 [DEBUG EDIT MODE] Navigating to previous node:', prevNode.id, 'with cursor at end')
              navigateToNode(prevNode.id, 'end')
            } else {
              console.log('🔵 [DEBUG EDIT MODE] No previous node found')
            }
          } else {
            console.log('🔵 [DEBUG EDIT MODE] No navigation available')
          }
        },
        onDownArrowAtLastLine: () => {
          console.log('🔴 [DEBUG EDIT MODE] DOWN ARROW at last line triggered for node:', props.node.id)
          if (navigation) {
            const nextNode = navigation.getNextNode(props.node.id)
            console.log('🔴 [DEBUG EDIT MODE] Next node:', nextNode)
            if (nextNode) {
              console.log('🔴 [DEBUG EDIT MODE] Navigating to next node:', nextNode.id, 'with cursor at start')
              navigateToNode(nextNode.id, 'start')
            } else {
              console.log('🔴 [DEBUG EDIT MODE] No next node found')
            }
          } else {
            console.log('🔴 [DEBUG EDIT MODE] No navigation available')
          }
...
outlineEmitter?.on('open-title-editor', (payload: unknown) => {
  ...
  console.log(`📝 [DEBUG EDITOR OPEN] Opening editor for ${nodeId} at ${cursorPosition}`)
