...
export class Layout {
  ...
  convert (treeData: TreeData, y = 0): Tree {
    ...
    if (treeData.children && treeData.children.length) {
      for (let i = 0; i < treeData.children.length; i++) {
        children[i] = this.convert(treeData.children[i] as TreeData, y + height)
      }
    }
  ...
  assignCoordinates (tree: Tree, treeData: TreeData): void {
    ...
    for (let i = 0; i < tree.c.length; i++) {
      ...
      if (child && treeData.children && treeData.children[i]) {
        this.assignCoordinates(child, treeData.children[i] as TreeData)
      }
    }
  }
  ...
  assignLayout<T extends TreeData> (tree: Tree, treeData: T, box?: Box): { result: T, boundingBox: Box } {
    ...
    for (let i = 0; i < tree.c.length; i++) {
      const child = tree.c[i]
      if (child && treeData.children && treeData.children[i]) {
        this.assignLayout(child, treeData.children[i] as T, box)
      }
    }
