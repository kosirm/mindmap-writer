...
1. **Priority 1**: Basic node properties (10 letters) - most frequently accessed
2. **Priority 2**: View-specific node properties (3 letters)
3. **Priority 3**: Map/document properties (6 letters)
4. **Priority 4**: Edge/link properties (7 letters)
5. **Priority 5**: Optional/metadata properties (2-letter combinations)

## Current Assignments

### Single Letters (26/26 USED)

**Priority 1: Basic Node Properties**
- `i` = NODE_ID
- `p` = NODE_PARENT_ID
- `t` = NODE_TITLE
- `c` = NODE_CONTENT
...
- Two-letter combinations: 676 (47 used, 629 available)
- Three-letter combinations: 17,576 (none used yet)
- **Total remaining**: 18,205 properties available

## Performance Benefits

1. **Hot path optimization**: Node operations (most frequent) use 1-byte property names
2. **Smaller storage**: 80-90% reduction in property name size
3. **Faster I/O**: Less data to read/write from IndexedDB
4. **Better caching**: More data fits in memory
5. **Faster sync**: Less data to transfer to Google Drive
